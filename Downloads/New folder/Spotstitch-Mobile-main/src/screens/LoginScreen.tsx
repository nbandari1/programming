...
export default function LoginScreen({ navigation }: { navigation: NativeStackNavigationProp<RootStackParamList> }) {
    ...
    <Background>
      ...
      <View style={{ flexDirection: "row", alignItems: "center" }}>
        ...
        <TouchableOpacity onPress={togglePasswordVisibility}>
          <Text style={{ fontSize: 16, color: "blue", marginLeft: 10 }}>{showPassword ? "Hide" : "Show"}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={[styles.row, styles.icons]}>
        {socialIconList.map((item) => {
          return <Icon key={item.name} name={item.name} containerStyle={styles.iconContainer} onPress={item.onPress} />;
        })}
      </View>
